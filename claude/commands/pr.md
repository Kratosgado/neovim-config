...
2. Ticket:
   - `create` → create a new Jira ticket in active sprint via `createJiraIssue`, assigned to `atlassianUserInfo`'s accountId, status **Code Review** (transition after create if needed), label either frontend/backend, summary from the diff. Use its key below.
