...
2. Ticket:
   - `create` → make a new Jira ticket via `createJiraIssue`, assigned to `atlassianUserInfo`'s accountId, status **Code Review** (transition after create if needed), label from branch scope, summary from the diff. Use its key below.
