...
M.load = function(callback)
    -- Try and load file
    local opt = config.opts.lang.kotlin.coverage_file
    ...
    local dir_prefix = Path:new(config.opts.lang.kotlin.dir_prefix .. "/").filename
    ...
    for _, pack in ipairs(packages) do
        ...
        for _, class in ipairs(pack.class) do
            local filename = Path:new(cwd .. "/" .. dir .. "/" .. class._attr.sourcefilename):absolute() -- with .kt
        ...
        for _, src_file in ipairs(pack.sourcefile) do
            ...
            if lines then
                for _, line in ipairs(lines) do
                    ...
                    local filename = Path:new(cwd .. "/" .. dir .. "/" .. src_file._attr.name):absolute()
...
M.sign_list = function(data)
    ...
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
        if vim.api.nvim_buf_is_loaded(b) then
            bufnr_by_abs[vim.fn.fnamemodify(vim.fn.bufname(b), ":p")] = b
        end
    end
    for fn, fdata in pairs(data.files) do
        local bufnr = bufnr_by_abs[fn]
